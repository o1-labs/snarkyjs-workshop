...
class TicTacToe extends SmartContract {
  ...
  // 1 | x  x  x
  // 2 | x  x  x
  @method async play(pubkey: PublicKey, player: Field, x: Field, y: Field) {
    ...
    Bool.or(player.equals(Field.one), player.equals(two)).assertEquals(true);
...
export async function main() {
  ...
  await Mina.transaction(player1, async () => {
    snappInstance
      .play(player2.toPublicKey(), two, Field.one, Field.zero)
  ...
  await Mina.transaction(player1, async () => {
    snappInstance
      .play(player1.toPublicKey(), Field.one, Field.one, Field.one)
      .catch((e) => console.log(e));
  })
    .send()
    .wait();
  ...
  await Mina.transaction(player2, async () => {
    snappInstance
      .play(player2.toPublicKey(), two, two, Field.one)
      .catch((e) => console.log(e));
  })
    .send()
    .wait();
  ...
  await Mina.transaction(player1, async () => {
    snappInstance
      .play(player1.toPublicKey(), Field.one, two, two)
      .catch((e) => console.log(e));
  })
    .send()
    .wait();
